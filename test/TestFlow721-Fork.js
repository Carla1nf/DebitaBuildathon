...
describe("Lock", function () {
  ...
  beforeEach(async function () {
    ...
    await contractERC721
      .connect(signerUser2)
      .approve(contractFactoryV2.target, 1);

    await contractERC721
      .connect(holderEQUAL)
      .approve(contractFactoryV2.target, 2);

    await contractERC721
      .connect(holderEQUAL)
      .approve(contractFactoryV2.target, 3);
  ...
  it("Create & Cancel Lending Offer  -- as Lender & Borrower with $EQUAL  (Lending: ERC-721, Collateral: ERC-721)", async function () {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      expect(await contractERC721.ownerOf(nftid)).to.be.equal(
        createdOfferAddress
      );
      // ------
      ...
      await expect(
        offerContract.connect(holderEQUAL).cancelOffer()
  ...
  it("Accept Offer, Create loan, Pay it & Claim it -- as Lender & Borrow with $EQUAL (Lending: ERC-721, Collateral: ERC-721)", async () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      await contractERC721
        .connect(signerUser2)
        .approve(createdOfferAddress, sendingID);
      ...
      let tx_Accept;
      if (i == 0) {
        tx_Accept = await offerContract
          .connect(signerUser2)
          .acceptOfferAsBorrower(1, sendingID);
      } else {
        tx_Accept = await offerContract
          .connect(signerUser2)
          .acceptOfferAsLender(1, sendingID);
      }
      ...
      await contractERC721
        .connect(borrower)
        .approve(loanContract.target, approveID);
      await contractERC20
        .connect(borrower)
    ...
    it("Create Loan, Default it & Claim it -- as Lender & Borrower with $EQUAL  (Lending: ERC-721, Collateral: ERC-721)", async () => {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        currentReceipt = createdReceipt[i];
        const createdOfferAddress = currentReceipt.logs[1].args[1];
        const offerContract = await contractOffersV2.attach(
          createdOfferAddress
        );
        const sendingID = i == 0 ? 4 : 5;
        await contractERC721
        ...
        */
        ...
        const createdLoanAddress = receipt_accept.logs[3].args[1];
        const loanContract = await contractLoansV2.attach(createdLoanAddress);
        await expect(loanContract.connect(lender).claimCollateralasLender()).to
          .be.reverted;
