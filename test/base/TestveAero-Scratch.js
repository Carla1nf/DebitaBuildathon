...
describe("Lock", function () {
  ...
  it("create Lock", async function () {
    ...
    const debitaLoanFactoryV2 = await LoanFactory.deploy();

    // Connect both factories
    await debitaLoanFactoryV2.setDebitaOfferFactory(factoryContract.target);
    await factoryContract.setLoanFactoryV2(debitaLoanFactoryV2.target);
    await ownerships.setDebitaContract(debitaLoanFactoryV2.target);
    await debitaLoanFactoryV2.setOwnershipAddress(ownerships.target);
    ...
    const loanAddress = receipt2.logs[4].args[1];
    ...
    const minterContract = await Minter.attach(
      "0xeB018363F0a9Af8f91F06FEe6613a751b2A33FE5"
    );
    const before = await veAeroContract.locked(20432);
    await time.increase(86400 * 14);
    await minterContract.updatePeriod();
    await loanContract.connect(holderEQUAL).claimRewards();
    const after = await veAeroContract.locked(20432);
