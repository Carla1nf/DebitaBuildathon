...
describe("Lock", function () {
  ...
  beforeEach(async function () {
    ...
    it("create Lock", async function () {
      ...
      const secondTokenId = Number(
        secondUserSupply[secondUserSupply.length - 1]
      );
    ...
    it("Create offer, and put lock as collateral", async function () {
      ...
      await contractOffersV2
        .connect(holderEQUAL)
        .acceptOfferAsLender(1, veEqualID);
    }),
    it("Create offer, and try to put lock as collateral with low amount", async function () {
      ...
      await contractVeEqual
        .connect(holderEQUAL)
        .approve(contractOffersV2.target, veEqualID_smallAmount);
      // locked amount of veEqualID_smallAmount == 100, but we need 1000
      await expect(
        contractOffersV2
          .connect(holderEQUAL)
          .acceptOfferAsLender(1, veEqualID_smallAmount)
      ).to.be.reverted;
    }),
    it("Accept offer as borrower", async function () {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const createdOfferAddress = receipt.logs[1].args[1];
        const contractOffers = await ethers.getContractFactory(
          "DebitaV2Offers"
        );
        contractOffersV2 = await contractOffers.attach(createdOfferAddress);
        ...
        if (i == 1) {
          await contractOffersV2
            .connect(holderEQUAL)
            .acceptOfferAsBorrower(1000, veEqualID_smallAmount);
        } else {
          await expect(
            contractOffersV2
              .connect(holderEQUAL)
              .acceptOfferAsBorrower(1000, veEqualID_smallAmount)
          ).to.be.revertedWith("Must be greater than veNFT value");
        }
      }
    });
});
