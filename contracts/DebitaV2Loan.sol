...
interface IDebitaLoanFactory {
    ...
    function emitUpdated(string memory _type) external;
}
...
contract DebitaV2Loan is ReentrancyGuard {
    ...
    constructor(
        ...
        uint256 totalAmountToPay = assetAmounts[0] +
    ...
    */
    ...
    function payDebt() public nonReentrant onlyActive {
        ...
        uint256 fee;
        uint256 interestFEE = IDebitaLoanFactory(debitaLoanFactory)
            .feeInterestLoan();
        if (loan.isAssetNFT[0]) {
            fee = (loan.nftData[2] * interestFEE) / 100;
            claimableAmount += loan.nftData[2] - fee;
        } else {
            uint256 interestPerPayment = ((loan.paymentAmount *
        ...
        IDebitaLoanFactory(debitaLoanFactory).emitUpdated("paymentMade");
    }
    ...
    function claimCollateralasLender() public nonReentrant onlyActive {
        ...
        if (isContractValid) {
            address voterAddress = getVoterContract_veNFT(
        ...
        IDebitaLoanFactory(debitaLoanFactory).emitUpdated("collateralClaimed");
    }
    ...
    function claimCollateralasBorrower() public nonReentrant onlyActive {
        ...
        try _ownerContract.ownerOf(m_loan.IDS[0]) returns (
        ...
        if (
            ...
            if (isLendingOffer) {
                ...
                uint256 sendingAmount = m_loan.isAssetNFT[0]
                    ? 1
                    : claimableAmount;
    ...
    function claimDebt() public nonReentrant {
        ...
        IDebitaLoanFactory(debitaLoanFactory).emitUpdated("claimedDebt");
    }
    ...
    */
    ...
    function increaseLock(uint256 duration) public onlyActive {
        ...
        veNFT(m_loan.assetAddresses[1]).increaseUnlockTime(
            m_loan.nftData[1],
            duration
        );
    ...
    function claimRewards() public onlyActive {
        ...
        IVotingEscrow.LockedBalance memory _locked = veNFT(
            m_loan.assetAddresses[1]
        ).locked(m_loan.nftData[1]);

        require(isContractValid, "Contract is not a veNFT");
        require(
            _timestamp < _locked.end || _locked.isPermanent,
            "Locked balance is over"
        );
        require(voterAddress != address(0), "Voter address is 0");
        require(
            msg.sender == _ownerContract.ownerOf(m_loan.IDS[1]),
